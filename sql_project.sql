...
the member name. */
SELECT DISTINCT Facilities.name AS court, CONCAT( Members.firstname,  ' ', Members.surname ) AS name
FROM Bookings
...
SELECT court,name,cost
FROM(SELECT Facilities.name AS court, CONCAT( Members.firstname,  ' ', Members.surname ) AS name,
(CASE WHEN Members.memid =0
THEN Bookings.slots * Facilities.guestcost
ELSE Bookings.slots * Facilities.membercost
END) AS cost
...
INNER JOIN Members ON Bookings.memid = Members.memid
INNER JOIN Facilities ON Bookings.facid = Facilities.facid
AND Bookings.starttime LIKE  '2012-09-14%') AS newlist
WHERE cost>30
